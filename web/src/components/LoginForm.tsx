...
export function LoginForm() {
  ...
  const urlError =
    urlParams.get("error") && urlParams.get("message")
      ? decodeURIComponent(urlParams.get("message")!)
      : null;
  const urlSuccess =
    urlParams.get("success") === "true" && urlParams.get("email")
      ? `Successfully logged in as ${decodeURIComponent(urlParams.get("email")!)}`
      : null;

  const [error, setError] = useState<string | null>(urlError);
  const [success, setSuccess] = useState<string | null>(urlSuccess);
  ...
  return (
    <div className="login-form">
      ...
      {!isOAuthConfigured && (
        ...
        >
          <strong>Note:</strong> Google OAuth is not configured. Please set up
          Google OAuth in your environment variables.
        </div>
      )}
